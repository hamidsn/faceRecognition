...
public class MainRepo {
    ...
    public void deleteItems(ArrayList<Integer> selectedList) {
        //selectedList
        HashMap<String, SimilarityClassifier.Recognition> currentSavedItems = readFromSP();
        HashMap<String, SimilarityClassifier.Recognition> newItems = new HashMap<>();
        Object[] r = currentSavedItems.keySet().toArray();

        for (int counter = 0; counter < currentSavedItems.size(); counter++) {
            if (!selectedList.contains(counter)) {
                newItems.put(r[counter].toString(), currentSavedItems.get(r[counter]));
            }
        }
        String jsonString = new Gson().toJson(newItems);
        SharedPreferences sharedPreferences = mContext.getSharedPreferences("HashMap", MODE_PRIVATE);
        SharedPreferences.Editor editor = sharedPreferences.edit();
        editor.putString("map", jsonString);
        editor.apply();
        Toast.makeText(mContext, "Item(s) deleted", Toast.LENGTH_SHORT).show();
