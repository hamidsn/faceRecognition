...
public class MainActivity extends AppCompatActivity implements MainCallBack {
    ...
    public void onBackPressed() {
        if (registered.size() != viewModel.readFromSP().size()) {
            AlertDialog.Builder builder = new AlertDialog.Builder(MainActivity.this);
            builder.setTitle("Unsaved faces");
            builder.setMessage("You have unsaved faces. Do you want to save them for future use?");
            builder.setPositiveButton("Yes", (dialogInterface, i) -> viewModel.insertToSP(registered, false, viewModel.readFromSP()));
   ...
   /* @Override
    ...
    @Override
    public void onSavedActionClick() {
        displayNameListView();
    }
    ...
    private void addFace() {
        ...
        descriptionBox.setHint("Additional info");
        ...
        builder.setPositiveButton("ADD", (dialog, which) -> {
            ...
            registered.put(input.getText().toString() + "@" + descriptionBox.getText().toString() + "#" + System.currentTimeMillis() / 10000, result);
            start = true;
...
/*    private  void clearNameList() {
    ...
    private void displayNameListView() {
        ...
        for (Map.Entry<String, SimilarityClassifier.Recognition> entry : registered.entrySet()) {
            ...
            String description = entry.getKey().substring(entry.getKey().indexOf("@") + 1, entry.getKey().indexOf("#"));
        ...
        builder.setMultiChoiceItems(names, null, (dialogInterface, itemNumber, isSelected) -> {
            if (isSelected) {
                selectedList.add(itemNumber);
            } else if (selectedList.contains(itemNumber)) {
                selectedList.remove(Integer.valueOf(itemNumber));
            }
        });
        builder.setPositiveButton("Delete selected", (dialogInterface, e) -> deleteItems(selectedList));
        AlertDialog dialog = builder.create();
        dialog.show();
    }

    private void deleteItems(ArrayList<Integer> selectedList) {
